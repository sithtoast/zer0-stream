defmodule Membrane.RTMPServer.ClientHandler do
  ...
  def handle_info({:client_timeout, _app, _stream_key}, state) do
    if not state.published? do
      Logger.warning("No demand made for RTMP client, terminating connection.")
  ...
  defp handle_data(data, state) do
    # Limit rescue to parsing untrusted bytes, not application callbacks.
    parsed =
      try do
        {:ok, MessageParser.parse_packet_messages(data, state.message_parser_state)}
      rescue
        _error -> {:error, :invalid_packet}
      end

    case parsed do
      {:ok, {messages, message_parser_state}} ->
        handle_messages(messages, message_parser_state, state)

      {:error, :invalid_packet} ->
        Logger.warning("Closing RTMP client: invalid packet")
        {:stop, :normal, handle_event(:connection_closed, state)}
    end
  end

  defp handle_messages(messages, message_parser_state, state) do
